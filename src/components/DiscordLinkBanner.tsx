...
const STATUS_MENSAGENS: Record<string, { tipo: 'ok' | 'erro'; texto: string }> = {
  ok: { tipo: 'ok', texto: 'Discord vinculado com sucesso! 🎉' },
  erro_login: { tipo: 'erro', texto: 'Faça login antes de vincular o Discord.' },
  erro_state: { tipo: 'erro', texto: 'Sessão de vínculo expirada. Tente novamente.' },
  ja_vinculado: { tipo: 'erro', texto: 'Esse Discord já está vinculado a outra conta.' },
  erro: { tipo: 'erro', texto: 'Não foi possível vincular o Discord. Tente novamente.' },
};

/**
 * Notificação lateral fixa que:
...
export function DiscordLinkBanner() {
  ...
  // Trata o retorno do /api/discord/callback (?discord=ok|erro|...).
  useEffect(() => {
    ...
    const info = STATUS_MENSAGENS[statusDiscord] ?? STATUS_MENSAGENS.erro;
    // Sincroniza com um sistema externo (a query string da URL) na montagem.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setFeedback(info);

    // Em caso de sucesso, reidrata o JWT para refletir o vínculo na hora.
    if (info.tipo === 'ok') {
      update();
    }

    // Limpa o parâmetro da URL para não repetir o feedback ao recarregar.
  ...
  const precisaVincular =
    status === 'authenticated' && session?.user && !session.user.discordLinked;

  if (!feedback && (!precisaVincular || dispensado)) return null;
        ...
        <div
          className={`flex items-start gap-2 rounded-xl border px-4 py-3 text-sm shadow-2xl backdrop-blur ${
            feedback.tipo === 'ok'
              ...
              : 'border-pink-subtle/40 bg-pink-subtle/15 text-pink-subtle'
          }`}
        >
          <span className="flex-1">{feedback.texto}</span>
          <button onClick={() => setFeedback(null)} className="shrink-0 opacity-70 hover:opacity-100" aria-label="Fechar">
